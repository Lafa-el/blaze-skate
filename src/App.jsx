...
export default function App() {
  ...
  const DashboardView = () => {
    ...
    const getSmartFocus = () => {
      // 如果今天没任务，直接提示休息
      if (!data.tasks || data.tasks.length === 0) return '今日休息 ☕';
      
      // 把所有任务名字拼成一个长字符串，方便统一检索
      const taskTexts = data.tasks.map(t => t.text || '').join(' ');
      
      // 关键词词典（你可以随时在这里增减关键词）
      const keywordMap = [
        { regex: /平衡|单脚|闭眼|胶带|木|姿态/i, label: '平衡与体感' },
        { regex: /跳|敏捷|壶铃|摇摆|爆发|抛掷|梯|切入/i, label: '爆发与敏捷' },
        { regex: /深蹲|硬拉|引体|划船|力量|负重|推/i, label: '力量储备' },
        { regex: /冲刺|HIIT|间歇|耐力|滑行板|单车|800米|400米/i, label: '专项耐力' },
        { regex: /拉伸|泡沫轴|筋膜枪|冰浴|放松/i, label: '恢复与预防' },
        { regex: /核心|平板|俄罗斯|悬垂|鸟狗|死虫子/i, label: '核心稳定性' },
        { regex: /冰感|静蹲|模仿|姿位|低姿|鸭子步/i, label: '专项体位' }
      ];

      const matches = [];
      keywordMap.forEach(item => {
        if (item.regex.test(taskTexts)) {
          matches.push(item.label);
        }
      });

      // 根据匹配结果返回最合适的标签
      if (matches.length === 0) return '综合训练';
      if (matches.length > 2) return '全方位综合';
      return matches.join(' & ');
    };

    // 运行算法，获取今天的智能标签！
    const todayFocus = getSmartFocus();

    // 比赛倒计时逻辑（保持原有稳定逻辑不变）
    const upcomingRaces = (data.races || [])
      .filter(r => new Date(r.date.replace(/-/g, '/')) >= new Date(currentTime.getFullYear(), currentTime.getMonth(), currentTime.getDate()))
    ...
    const daysToRace = nextRace 
      ? Math.ceil((new Date(nextRace.date.replace(/-/g, '/')) - new Date(currentTime.getFullYear(), currentTime.getMonth(), currentTime.getDate())) / (1000 * 60 * 60 * 24))
      : null;
    ...
    return (
      <div className="space-y-4 animate-in fade-in duration-300">
        <div className="grid grid-cols-2 gap-4">
          {/* 左侧：全新智能今日核心卡片 */}
          <div className={`${tc.cardBg} rounded-3xl p-5 shadow-sm border ${tc.borderLight} relative overflow-hidden`}>
            <div className={`text-xs font-bold ${tc.textMuted} mb-2`}>今日核心</div>
            <div className={`text-xl font-black ${tc.textHeading} leading-tight`}>{todayFocus}</div>
            <div className="absolute -right-2 -bottom-2 opacity-5"><Flame size={60} /></div>
          </div>
          
          {/* 右侧：下一场比赛卡片 */}
          <div className={`rounded-3xl p-5 bg-gradient-to-br ${tc.gradientPrimary} text-white shadow-md relative overflow-hidden`}>
            <div className="text-white/80 text-xs font-bold mb-2">下一场比赛</div>
            {nextRace ? (
              <>
                <div className="flex items-baseline gap-1">
                  <span className="text-3xl font-black">{daysToRace}</span>
                  <span className="text-sm font-bold opacity-80">天</span>
                </div>
                <div className="text-sm font-bold truncate mt-1 opacity-90">{nextRace.name}</div>
              </>
            ) : (
              <div className="text-sm font-bold opacity-80 mt-2">暂无赛事</div>
            )}
          </div>
        </div>
        ...
        {upcomingRaces.length > 1 && (
          <div>
            <h3 className={`text-sm font-bold ${tc.textMuted} mb-3 ml-1`}>其他即将到来</h3>
            <div className="space-y-2">
              {upcomingRaces.slice(1).map((race, idx) => (
                <div key={idx} className={`${tc.cardBg} p-4 rounded-2xl flex justify-between items-center shadow-sm border ${tc.borderLight}`}>
                  <div>
                    <div className={`font-bold ${tc.textHeading} text-sm`}>{race.name}</div>
                    <div className={`text-xs ${tc.textMuted} font-medium mt-0.5`}>{race.date.replace(/-/g, '/')}</div>
                  </div>
                  <div className={`text-xs font-bold bg-gray-100 ${tc.textPrimary} px-3 py-1.5 rounded-lg`}>
                    {Math.ceil((new Date(race.date.replace(/-/g, '/')) - new Date(currentTime.getFullYear(), currentTime.getMonth(), currentTime.getDate())) / (1000 * 60 * 60 * 24))} 天
                  </div>
                </div>
              ))}
            </div>
      ...
      <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/40 backdrop-blur-sm p-4 animate-in fade-in" onClick={() => setShowAccountManagement(false)}>
        <div className={`${tc.cardBg} w-full max-w-md rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[85vh]`} onClick={e => e.stopPropagation()}>
          ...
          <div className="p-5 overflow-y-auto space-y-6">
            ...
            {/* 2. 积分获取规则设置 */}
            <div>
              <h3 className={`text-sm font-bold ${tc.textMuted} mb-3 ml-1`}>🎯 积分获取规则</h3>
              <div className="space-y-3">
            ...
            {/* 3. 奖励兑换清单设置 */}
            <div>
              <h3 className={`text-sm font-bold ${tc.textMuted} mb-3 ml-1`}>🎁 奖励兑换清单</h3>
              <div className="space-y-2">
          ...
          <div className="p-4 border-t border-gray-100 flex gap-3 bg-gray-50/50">
            ...
            <button onClick={() => { setUser(null); updateData({}); }} className="px-4 py-3 bg-red-50 text-red-500 font-bold rounded-xl active:scale-95 transition-all">
              退出登录
            </button>
          </div>
        </div>
