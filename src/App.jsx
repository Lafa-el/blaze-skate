      ...
      <div className="space-y-3 pb-4">
        ...
        <div className="space-y-2">
          <AccordionHeader 
            ...
            summary={user?.isAnonymous ? (data.language === 'en' ? 'Guest ⚠️' : '游客模式 ⚠️') : (data.username || user?.email?.split('@')[0] || '已登录 🟢')} 
        ...
        <div className="space-y-2">
          ...
          {expandedSettingSection === 'security' && (
            <div className={`p-5 rounded-2xl border ${tc.borderLight} ${tc.cardBg} animate-in fade-in slide-in-from-top-2 duration-200`}>
              ...
              {pinError && <div className={`text-xs ${pinError.includes('✅') ? 'text-green-600' : 'text-red-500'} font-bold mt-2`}>{pinError}</div>}
            </div>
          )}
        </div>
