...
import SelectedPlanHeader from './features/plans/SelectedPlanHeader';
import SelectedPlanTaskItem from './features/plans/SelectedPlanTaskItem';
...
const translations = {
  zh: {
    ...
    weeklyReport: '本周报告',
    printReport: '打印报告',
    weeklyTrainingReport: '本周训练报告',
    generatedDate: '生成日期',
    athleteLabel: '运动员',
    trainingPlanSummary: '训练计划摘要',
    dailyExecutionSummary: '今日执行摘要',
    dailyTasksCurrentNote: '今日任务反映当前今天的执行列表。',
    recentPbProgress: '最近 PB 进展',
    generatedByBlaze: '由 Blaze Skate Training 生成。',
    planTasksLabel: '计划任务',
  ...
  en: {
    ...
    weeklyTrainingReport: 'Weekly Training Report',
    generatedDate: 'Generated Date',
    athleteLabel: 'Athlete',
    trainingPlanSummary: 'Training Plan Summary',
    dailyExecutionSummary: 'Daily Execution Summary',
    dailyTasksCurrentNote: 'Daily Tasks reflect the current today list.',
    recentPbProgress: 'Recent PB Progress',
    generatedByBlaze: 'Generated by Blaze Skate Training.',
...
export default function App() {
  ...
  const athleteDisplayName = data.username || (user?.isAnonymous ? 'Guest Skater' : user?.email?.split('@')[0] || '');
  const selectedGoalDetail = (data.competitionGoalsV1 || []).find((item) => item?.id === selectedGoalForDetails) || null;
  const weeklyReportWeekStartDate = getWeekStartDateString(currentTime);
  const weeklyReportData = getWeeklyTrainingReportData(data, weeklyReportWeekStartDate);
            ...
            {selectedPlan && (
              <>
                <SelectedPlanHeader
                  ...
                  onOpenWeeklyReport={openWeeklyReportModal}
                  onEditPlan={() => openEditPlanModal(selectedPlan)}
                  onArchivePlan={() => archivePlan(selectedPlan)}
                />
                        ...
                        <div className="space-y-2">
                          {(day.tasks || []).map(task => (
                            <SelectedPlanTaskItem
                              ...
                              t={t}
                              tc={tc}
                              taskDailyStatus={getPlanTaskDailyStatus(task, day.date, data.tasks || [], currentDateStr)}
                              onToggleComplete={() => togglePlanTaskCompletion(selectedPlan, task, !task.completed)}
                              onAddToToday={() => addPlanTaskToToday(task)}
                              onEdit={() => openEditPlanTaskModal(task, day.date)}
                            />
                          ))}
                        </div>
                      </div>
    ...
    <div className={`min-h-screen ${tc.appBg} ${tc.appText} font-sans max-w-md mx-auto shadow-2xl relative pb-24 transition-colors duration-300`}>
      ...
      <GoalDetailModal
        ...
        onEditGoal={() => {
          ...
          closeGoalDetailModal();
          openEditGoalModal(selectedGoalDetail);
        }}
        onArchiveGoal={() => {
          if (!selectedGoalDetail) return;
